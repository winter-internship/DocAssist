...
async function loadDocument() {
  try {
    ...
    if (doc.analysis) {
        ...
        analysis.rules = doc.analysis.rules || [];
        
        if (doc.analysis.paragraphs && doc.analysis.paragraphs.length > 0) {
             originalParagraphs.value = doc.analysis.paragraphs.map((p: any) => p.original || "");
             assistedParagraphs.value = doc.analysis.paragraphs.map((p: any) => ({
                 easy: p.easy || "",
                 bullets: p.bullets || []
             }));
        }
    ...
    if (doc.glossary_terms) {
        analysis.terms = doc.glossary_terms.map((t: any) => ({
            term: t.term,
...
async function loadChatSession() {
    try {
        ...
        // Find session for this document
        const exist = sessions.find((s: any) => s.document_id === docId.value);
        
        if (exist) {
            currentSessionId.value = exist.id;
            const msgRes = await chatService.getMessages(exist.id);
            chat.value = msgRes.data.map((m: any) => ({
                id: m.id,
                role: m.role,
                text: m.content
            }));
        } else {
            const createRes = await chatService.createSession(`Chat - ${docMeta.title}`, docId.value);
...
function seedQuestions() {
  const seeds = ["이 문서의 핵심 내용은 무엇인가요?", "중요한 주의사항이 있나요?", "용어 설명을 해주세요."];
...
async function ask() {
  ...
  if (!currentSessionId.value) {
      alert("채팅 세션이 초기화되지 않았습니다.");
      return;
  }

  // Optimistic update
  const userMsgId = "tmp_" + Date.now();
  chat.value.push({ id: userMsgId, role: "user", text: q });
  const originalQ = q; // Save for API call
  question.value = "";
  ...
  try {
      const res = await chatService.askQuestion(currentSessionId.value, originalQ);
      const ans = res.data;
      chat.value.push({
          id: ans.id,
          role: "assistant",
          text: ans.content,
          // citePara: ans.citations?.[0]?.para // If backend supports citation mapping
      });
  } catch (e) {
      console.error(e);
      chat.value.push({ id: "err_" + Date.now(), role: "assistant", text: "답변 생성 중 오류가 발생했습니다." });
  }
