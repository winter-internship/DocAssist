...
function setFontSize(next: "sm" | "md" | "lg") {
  ui.fontSize = next;
...
async function save() {
  try {
    ...
    // API 호출로 저장
    await authStore.updateUser({ profile_settings: payload });
...
onMounted(async () => {
  ...
  if (authStore.user) {
    ...
    // 설정 복원 (DB > LocalStorage)
    const settings = authStore.user.profile_settings;
    if (settings) {
      if (settings.assist) Object.assign(assist, settings.assist);
